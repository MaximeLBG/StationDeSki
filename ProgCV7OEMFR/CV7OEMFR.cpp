...
int CV7OEMFR::getTypeTrame()
{
        ...
        else       // Sinon si aucune des deux trames ont été touvées, on renvoi 0
            return 0;
    #endif
        ...
        else if (this->trame.indexOf("$WIXDR")!=-1)     //On chercher "$WIXDR" dans la trame de la classe
        {
            ...
            return 0;       // Sinon si aucune des deux trames ont été touvées, on renvoi 0
    #endif
}
...
#ifdef Lorawan
   StringSplitter* laTrame = new StringSplitter(this->trame,',',6);     //On déclare un pointeur sur la classe StringSplitter
